...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  ">
   ...
   <div class="d-flex justify-content-center form-check form-switch mb-3">
    ...
    <!-- Project Cover -->
    <div class="position-relative" id="${wrapperId}">
    ...
    <img src="${serverStorage}${projectPhoto}" alt="${projectName}" style="
      max-width: 400px;
      width: 100%;
      display: block;
    " />
    ...
    <!-- Project Info -->
    <div style="padding: 1rem;" class="text-break">
      <h3 style="white-space: pre-line; margin: 0 0 0.5rem 0; color: #333;">${projectName}</h3>
      <!-- Description Button -->
<button id="openDescription-${userSlug}-${projectSlug}" class="btn btn-primary mb-3">
...
<div id="markdownOverlay-${userSlug}-${projectSlug}" style="
  position: fixed;
  top: 0;
  left: 0;
  width: 100vw;
    ...
    " class="scrollbar-thin">
      ...
      <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
        ...
        <div>
          ...
          <p style="margin: 0; color: #666; font-size: 0.8rem;">${userEmail}</p>
        </div>
      </div>

      <div class="d-flex text-center align-items-center w-100 bg-dark mt-1 rounded">
                 <a class="nav-link m-2" href="#" id="heartToggle-${userSlug}-${projectSlug}">
                    <svg id="heartIcon-${userSlug}-${projectSlug}" xmlns="http://www.w3.org/2000/svg" width="40" height="40"
                  ...
                  <a class="nav-link text-light" href="#" style="cursor: default;">
                    ...
                    <p class="text-light m-0" id="numberOfViews-${userSlug}-${projectSlug}">0</p>
                  </div>
              </div>
    </div>
  </div>
        ...
        for (let i = 0; i < arrayUserProjects.length; i++) {
          ...
          projectsTable.insertAdjacentHTML("beforeend", `
            <div class="p-2 d-flex justify-content-center align-items-center text-center flex-column motherCard" style="height: auto; max-width: 600px; width: 90%;">
              ...
              <div id="${wrapperId}" class="position-relative w-100 bg-black bg-gradient rounded" style="aspect-ratio: 16 / 9;">
                ...
                <img src="${serverStorage}${project.photo}" id="${imageId}" class="w-100 h-100 object-fit-cover rounded p-1" alt="">
              </div>

              <div class="d-flex text-center align-items-center w-100 bg-dark mt-1 rounded">
                 <a class="nav-link m-2" href="#" id="heartToggle-${slugUser}-${project.slug}">
                    <svg id="heartIcon-${slugUser}-${project.slug}" xmlns="http://www.w3.org/2000/svg" width="40" height="40"
                          fill="white" class="bi bi-heart" viewBox="0 0 16 16">
                            <path
                             d="m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143q.09.083.176.171a3 3 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15" />
                      </svg>
           ...
           <div class="d-flex flex-column text-start rounded w-100 h-auto m-1 bg-black bg-opacity-50 p-3">

 

  <!-- Project Info Section -->
 
    <h3 style="white-space: pre-line; margin: 0 0 0.5rem 0;" class="text-light">${nameProject}</h3>
    <div id="markdown-content-${slugUser}-${project.slug}" class="scrollbar-thin p-1" style="margin: 0 0 1rem 0; font-size: 0.9rem; color: white; max-height:250px; overflow-y:scroll;"></div>
    ...
    <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
        ...
        <div>
          ...
          <p style="margin: 0; color: #666; font-size: 0.8rem;"></p>
        </div>
      </div>
    ...
    document.getElementById('inputSearchProjectsUser').addEventListener('input', function () {
      ...
      cards.forEach(card => {
        const projectName = card.querySelector('h6')?.textContent.toLowerCase().trim();
        if (searchText.trim() == "") {
