...
div {
  ...
  font-weight: 200;
      ...
      function injectScriptsOnly() {
        ...
        const useModule = document.getElementById('useModule')?.checked;
        const typeAttr = useModule ? ' type="module"' : '';
      
        ['iframeA', 'iframeB'].forEach(frameId => {
          const iframe = document.getElementById(frameId);
          if (!iframe?.contentDocument) return;
      
          const doc = iframe.contentDocument;
      
          // Create a template string of scripts
          const scriptsHTML = jsInputs
            .map(src => `<script src="${src}"${typeAttr}></script>`)
            .join('\n');
      
          // Inject the scripts as raw HTML appended to body
